...
const deviceFingerprintSchema = new mongoose.Schema({
  ...
  // Location Profile for Geographic Analysis
  locationProfile: {
  ...
  // Analytics Profile for Behavioral Analysis
  analytics: {
...
deviceFingerprintSchema.pre('save', function (next) {
  // Ensure all required nested objects exist before accessing their properties
  if (!this.securityProfile) this.securityProfile = {};
  if (!this.networkProfile) this.networkProfile = {};
  if (!this.behaviorProfile) this.behaviorProfile = {};
  if (!this.deviceSignature) this.deviceSignature = {};
  if (!this.locationProfile) this.locationProfile = {};
  if (!this.analytics) this.analytics = {};
  if (!this.threatIntelligence) this.threatIntelligence = {};
  if (!this.bangladeshProfile) this.bangladeshProfile = {};
  ...
  const humanBehaviorScore = this.behaviorProfile.humanBehaviorScore || 50; // Default value
  ...
  const sessionDuration = this.behaviorProfile.sessionDuration || 300; // Default 5 minutes
  if (reportingFrequency > 10) anomaly += 8; // Too many reports
  if (sessionDuration < 30) anomaly += 5; // Very short sessions
  ...
  if (screenResolution === '800x600' || screenResolution === '1024x768') {
    anomaly += 8; // Suspicious resolutions
  }
